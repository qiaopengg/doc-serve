...
export class RelationshipParser {
  ...
  /**
   * 解析关系文件并返回简化的 Map（兼容旧版 API）
   * 支持多种关系类型：hyperlink, image, header, footer, footnotes, endnotes, comments
   */
  parseRelationships(xml: string): Map<string, string> {
    const relationships = this.parse(xml)
    const out = new Map<string, string>()
    
    for (const [id, rel] of relationships.entries()) {
      if (rel.type.includes('/hyperlink') ||
          rel.type.includes('/image') ||
          rel.type.includes('/header') ||
          rel.type.includes('/footer') ||
          rel.type.includes('/footnotes') ||
          rel.type.includes('/endnotes') ||
          rel.type.includes('/comments')) {
